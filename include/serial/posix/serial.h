...
public:
	...
	serial_t( const string_t& path, uint baud=0, const ulong& _size=NODEPP_CHUNK_SIZE )
	: file_t( path, nullptr, _size ){ 
		auto self = type::bind(this); set_baud_rate( get_fd(), baud );
		process::add([=](){ self->onOpen.emit(); return -1; });
	}
...
namespace nodepp { namespace serial {
	...
	inline promise_t<serial_t,except_t> 
	connect( const string_t& path, uint baud=0, const ulong& _size=NODEPP_CHUNK_SIZE ){ 
	...
	inline expected_t<serial_t,except_t> 
	await( const string_t& path, uint baud=0, const ulong& _size=NODEPP_CHUNK_SIZE ){ 
		return connect( path, baud, _size ).await(); 
	}

    /*─······································································─*/

	inline expected_t<ptr_t<string_t>,except_t> 
	scan(){
